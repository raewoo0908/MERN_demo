...
async function run() {
  await connectLocal();
  console.log('Aggregating agg_dow_hour_usertype from hourly_usage…');
  const start = Date.now();

  const rows = await HourlyUsage.aggregate<{
    _id: { dow: number; hour: number; userType: string };
    tripCount: number;
    distanceM: number;
    durationMin: number;
  }>([
    ...
    { $sort: { '_id.dow': 1, '_id.hour': 1, '_id.userType': 1 } },
  ]).allowDiskUse(true);
  ...
  await replaceCollection('agg_dow_hour_usertype', docs);

  const elapsed = ((Date.now() - start) / 1000).toFixed(1);
  console.log(
    `\ndone: ${docs.length} rows, sum(tripCount)=${sumTripCount(docs).toLocaleString()} (${elapsed}s)`,
  );
  printSample('agg_dow_hour_usertype', docs);
  await disconnect();
}
